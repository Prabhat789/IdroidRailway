...
import com.pktworld.railway.adapter.RouteAdapter;
import com.pktworld.railway.model.TrainRouteResponse;
    ...
    public void onClick(View v) {
        if (v == btnSubmit){
            if (editTrainNumber.getText().toString().trim().isEmpty() || editTrainNumber.getText().toString().length() == 0){
                Utils.showToastMessage(TrainRouteActivity.this, "Enter Train Number");
            }else {
                if (Utils.isConnected(TrainRouteActivity.this)){
            ...
            public void onResponse(TrainRouteResponse response) {
                try {
                    ...
                    if (response.getResponse_code().equals("200")){
                        ...
                        Utils.setListViewHeightBasedOnChildren(listRoute);
                    }else{
                        llDatacont.setVisibility(View.GONE);
                        listRoute.setVisibility(View.GONE);
                        Utils.showToastMessage(TrainRouteActivity.this,getString(R.string.empty_response));
                    }
