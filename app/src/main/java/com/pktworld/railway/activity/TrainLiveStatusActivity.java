...
import com.pktworld.railway.adapter.TrainRouteAdapter;
import com.pktworld.railway.model.LiveStatus;
...
public class TrainLiveStatusActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        if (v == btnDate){
            ...
            if (validate()){
                if (Utils.isConnected(TrainLiveStatusActivity.this)){
    ...
    private boolean validate(){
        ...
        if (editSearch.getText().toString().trim().length() == 0 || editSearch.getText().toString().trim().isEmpty()){
            Utils.showToastMessage(TrainLiveStatusActivity.this, "Please enter Train Number");
            return false;
        }else if (btnDate.getText().toString().equalsIgnoreCase("date")){
            Utils.showToastMessage(TrainLiveStatusActivity.this,"Please select journey date");
            return false;
        }else {
            return true;
        }
            ...
            public void onResponse(LiveStatus response) {
                try {
                    ...
                    if (response.getResponse_code().equals("200")){
                        ...
                        Utils.showToastMessage(TrainLiveStatusActivity.this,getString(R.string.empty_response));
                    }
