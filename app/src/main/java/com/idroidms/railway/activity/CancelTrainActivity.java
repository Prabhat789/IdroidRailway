...
import com.idroidms.railway.adapter.CancelledTrainAdapter;
import com.idroidms.railway.model.CancelTrainResponse;
...
public class CancelTrainActivity extends AppCompatActivity implements View.OnClickListener
{
    ...
    public void onClick(View v) {
        if (v == btnSearch){
            if (validate()){
                if (AppUtils.isConnected(CancelTrainActivity.this)){
    ...
    private boolean validate(){
        if (btnDate.getText().toString().equalsIgnoreCase("Date")){
            AppUtils.showToastMessage(CancelTrainActivity.this,"Please select Date");
            return false;
        }else {
            return true;
        }
            ...
            public void onResponse(CancelTrainResponse response) {
                ...
                try {
                    ...
                    if (response.getResponse_code().equals("200")){
                        ...
                        AppUtils.setListViewHeightBasedOnChildren(listCancelTrains);
                        if (mProgressDialog.isShowing()){
                            mProgressDialog.dismiss();
                        }
                    }else{
                        ...
                        AppUtils.showToastMessage(CancelTrainActivity.this,getString(R.string.empty_response));
                    }
